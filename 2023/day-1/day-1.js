...
const finalValuePartOne = numbers
  .map((value) => {
...
const replaceStringNumbers = data.map((value) => {
  ...
  for (let i = 0; i < valueLookup.length; i++) {
    const replaceKey = findAlphaNumbers(
      value,
      valueLookup[i][0],
      valueLookup[i][1],
    );
  ...
  for (let i = 0; i < detectedNumbers.length; i++) {
    for (let j = 0; j < detectedNumbers[i].length; j++) {
      value = buildNew(
        value,
        detectedNumbers[i][j][0],
        detectedNumbers[i][j][1],
      );
    }
  }
  return value;
...
function buildNew(
  stringThatNeedsReplacing,
  indexOfValueBeingReplaced,
  valueToInput,
) {
  return [...stringThatNeedsReplacing]
    .map((value, i) => {
